...
class LeaveManager(models.Manager):
	...
	def all_pending_leaves(self):
		...
		'''
		return super().get_queryset().filter(status = '申請中').order_by('-created')# applying FIFO 




	def all_cancel_leaves(self):
		return super().get_queryset().filter(status = 'キャンセル').order_by('-created')




	def all_rejected_leaves(self):
		return super().get_queryset().filter(status = '却下').order_by('-created')
	...
	def all_approved_leaves(self):
		...
		'''
		return super().get_queryset().filter(status = '承認済')

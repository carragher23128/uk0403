...
def leave_creation(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			messages.success(request,'申請が完了しました',extra_tags = 'alert alert-success alert-dismissible show')
			return redirect('dashboard:createleave')

		messages.error(request,'申請できません．日付を確認してください',extra_tags = 'alert alert-warning alert-dismissible show')
		return redirect('dashboard:createleave')
...
def approve_leave(request,id):
	...
	messages.error(request,' {0}さんの予定を承認しました'.format(user),extra_tags = 'alert alert-success alert-dismissible show')
	return redirect('dashboard:userleaveview', id = id)

###################################################
# 追加＿承認
def cansel_leave_emp(request,id):
	if not (request.user.is_authenticated):
	# if not (request.user.is_superuser and request.user.is_authenticated):
		return redirect('/')
	leave = get_object_or_404(Leave, id = id)
	...
	leave.save()
	messages.error(request,'キャンセルされました'.format(user),extra_tags = 'alert alert-success alert-dismissible show')
	return redirect('dashboard:userleaveview', id = id)


def create_leave_emp(request,id):
	if not (request.user.is_authenticated):
	# if not (request.user.is_superuser and request.user.is_authenticated):
		return redirect('/')
	leave = get_object_or_404(Leave, id = id)
	...
	messages.error(request,'再申請されました'.format(user),extra_tags = 'alert alert-success alert-dismissible show')
	return redirect('dashboard:userleaveview', id = id)
...
def unapprove_leave2(request, id):
	if not (request.user.is_authenticated and request.user.is_superuser):
		return redirect('/')
	leave = get_object_or_404(Leave, id = id)
	leave.unapprove_leave
	#return render(request,'dashboard:leavesrejected')
	#return render('dashboard:userleaveapprove')
	messages.success(request,'申請中リストに戻しました',extra_tags = 'alert alert-success alert-dismissible show')
	return redirect('dashboard:approvedleaveslist') #redirect to unapproved list
...
def cancel_leave(request,id):
	...
	messages.success(request,'キャンセルされました',extra_tags = 'alert alert-success alert-dismissible show')
...
def reject_leave(request,id):
	...
	leave = get_object_or_404(Leave, id = id)
	leave.reject_leave
	messages.success(request,'却下されました',extra_tags = 'alert alert-success alert-dismissible show')
...
def unreject_leave(request,id):
	...
	leave.is_approved = False
	leave.save()
	messages.success(request,'申請中に戻しました ',extra_tags = 'alert alert-success alert-dismissible show')
...
def you(request):
	#return redirect('https://b0aa600df9934664bf4b.sharepoint.com.mcas.ms/sites/hannbai-yusyouka')

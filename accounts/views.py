...
def users_list(request):
	...
	# employees = UserAddForm.username.all()
	employees = User.objects.all()
	kanrisya = str(request.user)
	return render(request,'accounts/users_table.html',{'employees':employees,'title':'Users List','kanrisya':kanrisya})


def users_unblock(request,id):
	user = get_object_or_404(User,id = id)
	# emp = Employee.objects.filter(user = user).first()
	emp = User.objects.filter(username = emp).first()
	emp.delete()
...
def users_block(request,id):
	user = get_object_or_404(User,id = id)
	#emp = User.objects.filter(username = user)
	emp =  User.objects.filter(username = user)
	emp.delete()
	#emp = emp.first.delete()
